...
class ParserException : public std::exception {
   ...
   public:
    ...
    const char* what() const noexcept override {
        formatted_message = std::format("ParserException: [{}:{}] {} but next token is {}",
                                        function_name, location.line(), message, next_token.lexeme);

        return formatted_message.c_str();
    }

    bool operator==(const ParserException& rhs) {
        return this->what() == rhs.what();  // Compare based on the exception message
    }
};
